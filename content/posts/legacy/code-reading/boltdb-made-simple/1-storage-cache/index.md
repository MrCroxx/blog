---
title: "(Chinese) 深入浅出 boltdb —— 0x01 存储与缓存"
date: "2021-01-05"
summary: "深入浅出 boltdb —— 0x01 存储与缓存"
...
> 注意：
>
> boltdb在进行映射时，mmap大小可能超过数据库文件大小。在访问超出文件大小的mmap部分时会引起`SIGBUS`异常。为了避免访问到超出文件的部分，同时尽可能减少对底层文件大小的的不必要的增长，boltdb选择的是在事务提交时按需增长底层文件大小的策略。其具体实现详见本系列后续文章[深入浅出 boltdb —— 0x04 事务](/posts/code-reading/boltdb-made-simple/4-transaction/)。
