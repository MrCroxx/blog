---
title: "(Chinese) 深入浅出 boltdb —— 0x03 bucket & cursor"
date: "2021-01-20"
summary: "深入浅出 boltdb —— 0x03 bucket & cursor"
...
在[深入浅出 boltdb —— 0x02 B+Tree](/posts/code-reading/boltdb-made-simple/2-b+tree-copy/)中，笔者介绍了boltdb中B+Tree的实现。boltdb将B+Tree进一步封装成了bucket以便用户使用。
...
3. 如果当前bucekt中有更新，则调用根节点的`spill`方法，该方法会递归调用整棵B+Tree中已实例化的node的`spill`方法（详见[深入浅出 boltdb —— 0x02 B+Tree](/posts/code-reading/boltdb-made-simple/2-b+tree/)中介绍的node的`spill`方法的实现）。
