...
- **安全性：** Raft中关键的安全性是**图3**中的状态机安全性（State Machine Safety Property）：如果任意服务器将一个特定的日志条目应用到了其状态机中，那么不会有应用了有相同索引号的其他指令的日志条目的服务器。[章节5.4](#54-)描述了Raft如何确保这一性质；其解决方案包括对[章节5.2](#52-)中描述的选举机制的一个额外的约束。
...
Raft服务器使用远程过程调用（remote procedure call，RPC）通信，且基本的共识算法仅需要两种RPC。